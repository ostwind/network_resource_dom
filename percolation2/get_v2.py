...
def next_v(table):
    ...
    #active_table = undecided_v(table)
    ...
    avg_prob = sum(dist.values())/len(dist)
    v_getter = VoseAlias(dist)
    output_v = v_getter.alias_generation()
    #output_v = pick_best(dist) #currently picks vertex w/ best probability automatically
                            #to probablistically generate vertex use above comment

    #if output_v == 0:
    #   output_v = v_getter.alias_generation()

    #print output_v, p[output_v]

    if table[output_v][0] != 0:
        for v,data in table.iteritems():
            print v, data
        print output_v, table[output_v]
        raise Exception('ff')

    #print output_v, table[output_v][4], table[output_v][4]-avg_prob, len(dist)
    #print table[output_v]

    return output_v#str(sample(distribution,probability))

import operator
def pick_best(dist):
    return max(dist.iteritems(),key=operator.itemgetter(1))[0]
